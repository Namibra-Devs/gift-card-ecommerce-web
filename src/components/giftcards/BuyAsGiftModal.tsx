...
const BuyAsGiftModal: React.FC<GiftCardModalProps> = ({
  isOpen,
  onClose,
  giftCard,
}) => {
      ...
      <div className="w-[450px] bg-white rounded-tr-3xl rounded-tl-3xl md:rounded-lg shadow-lg overflow-hidden">
        <div className="flex flex-col-reverse md:flex-row items-center justify-between gap-7 bg-white md:bg-greylight p-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="h-7 w-7 hidden md:flex items-center justify-center rounded-full bg-white text-grey"
          >
        ...
        <form onSubmit={handleSubmit} className="-mt-8 md:mt-2">
          ...
          <div className="bg-white md:bg-greylight p-4 flex justify-end space-x-4 mt-0 md:mt-4">
            <button
              type="button"
              onClick={onClose}
              className="hidden md:block px-8 py-3 bg-white text-greynormal hover:bg-greylightactive rounded"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="min-w-full md:min-w-0 px-8 py-3 bg-black text-white rounded"
              disabled={loading}
            >
